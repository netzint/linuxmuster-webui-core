...
from wu.gate.gate import WorkerGate
from wu.gate.session import Session
from wu.gate.worker import WorkerError
from wu.util import str_fsize
...
class GateMiddleware(object):
    ...
    def open_session(self, env, **kwargs):
        """
        Creates a new session for the :class:`wu.http.HttpContext`
        """
        max_sessions = wu.config.data['max_sessions']
    ...
    def handle(self, http_context):
        ...
        if not session and wu.dev_autologin:
