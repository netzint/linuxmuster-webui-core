...
import wu
from wu.api.http import SocketEndpoint
from wu.http import HttpMiddlewareAggregator, HttpContext
from wu.auth import AuthenticationMiddleware, AuthenticationService
from wu.routing import CentralDispatcher
from wu.log import set_log_params, init_log_forwarding
...
class Worker(object):
    def __init__(self, stream, gate):
        ...
        self.context = Context(parent=wu.context)
    ...
    def run(self):
        if self.gate.restricted:
            restricted_user = wu.config.data['restricted_user']
    ...
    def handle_http_request(self, rq):
        ...
        try:
            ...
            # Generate response
            content = HttpMiddlewareAggregator([self.handler]).handle(http_context)
            # ---
