...
def detect_platform():
    ...
    dist = ''
    (mwu, min, patch) = pyplatform.python_version_tuple()
    mwu = int(mwu)
    min = int(min)
    if (mwu * 10 + min) >= 26:
...
def init():
    import wu
    wu.version = detect_version()
    if wu.platform is None:
        wu.platform_unmapped, wu.platform = detect_platform()
    else:
        logging.warn('Platform ID was enforced by commandline!')
        wu.platform_unmapped = wu.platform
    wu.platform_string = detect_platform_string()
