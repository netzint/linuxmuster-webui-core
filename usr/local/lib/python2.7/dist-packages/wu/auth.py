...
import wu
from wu.api.http import BaseHttpHandler
from wu.security.verifier import ClientCertificateVerificator
from wu.util import public
...
class AuthenticationService(object):
    ...
    def get_provider(self):
        provider_id = wu.config.data['auth'].get('provider', 'os')
    ...
    def check_sudo_password(self, username, password):
        if not wu.config.data['auth'].get('allow_sudo', False):
            return False
    ...
    def client_certificate_callback(self, connection, x509, errno, depth, result):
        ...
        if not wu.config.data['ssl']['client_auth']['force']:
            return True
        user = ClientCertificateVerificator.get(wu.context).verify(x509)
        return bool(user)
...
class authorize(object):
    ...
    def check(self):
        username = wu.worker.context.identity
        provider = AuthenticationService.get(wu.worker.context).get_provider()
        for permission in [
            p
            for permission_provider in PermissionProvider.all(wu.worker.context)
