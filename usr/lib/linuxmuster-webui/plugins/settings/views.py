...
class Handler(HttpPlugin):
    ...
    def handle_api_generate_client_certificate(self, http_context):
        ...
        ca_key = load_privatekey(FILETYPE_PEM, open(wu.config.data['ssl']['certificate']).read())
        ca_cert = load_certificate(FILETYPE_PEM, open(wu.config.data['ssl']['certificate']).read())
        cert = X509()
    ...
    def handle_api_generate_server_certificate(self, http_context):
        etc_dir = '/etc/webui'
        certificate_path = '%s/webui.pem' % etc_dir
        ...
        cert.get_subject().commonName = 'webui'
        cert.set_pubkey(key)
        cert.set_serial_number(random.getrandbits(8 * 20))
        cert.gmtime_adj_notBefore(0)
        cert.gmtime_adj_notAfter(10 * 365 * 24 * 60 * 60)
