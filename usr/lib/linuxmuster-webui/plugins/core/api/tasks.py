...
from wu.util import BroadcastQueue
from wu.plugins.core.api.push import Push
...
class Task(object):
    ...
    def push(self, plugin, message):
        """
        An interface to :class:`wu.plugins.core.api.push.Push` usable from inside the task's process
        """
        self.pipe.put({
            'type': 'push',
            'plugin': plugin,
