...
from wu.auth import AuthenticationService, SudoError
from wu.plugins import PluginManager

from wu.api.endpoint import endpoint
from wu.plugins.core.api.sidebar import Sidebar
from wu.plugins.core.api.navbox import Navbox
...
class Handler(HttpPlugin):
    ...
    def handle_api_identity(self, http_context):
        return {
            'identity': {
                ...
                'elevation_allowed': wu.config.data['auth'].get('allow_sudo', False),
            ...
            'machine': {
                'name': wu.config.data['name'],
                'hostname': socket.gethostname(),
            },
            'color': wu.config.data.get('color', None),
        }
    ...
    def handle_api_web_manifest(self, http_context):
        return {
            'short_name': wu.config.data['name'],
            'name': '%s (%s)' % (wu.config.data['name'], socket.gethostname()),
